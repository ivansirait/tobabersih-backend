...
export const getLaporan = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    // 🔥 PERBAIKAN: Kembalikan konversi BigInt ke String agar JSON.stringify tidak error
...
Kabupaten Toba
    ...
    return res.status(201).json({
      ...
      data: {
        ...
        userId: dataBaru.userId ? dataBaru.userId.toString() : null,
        // Optional chaining aman jika sewaktu-waktu DB tidak mereturn locationId
        locationId: (dataBaru as any).locationId ? (dataBaru as any).locationId.toString() : null,
      },
    });
  } catch (error: any) {
